...
- 📱 "就看一眼B�? �?3小时后："卧槽都凌晨了？！"
- 🎮 "打一局就睡" �?天亮了："我好像通宵�?.."
- 🛒 "随便逛逛淘�? �?购物车："您有137件商品待支付"
- 📺 "刷一会抖�? �?老板�?你被开除了"

**如果你也有这些症状，恭喜你，你需�?ZenBlock�?*

> 这是一个纯粹的Vibe Coding项目�?不要指望它有任何质量保证或售后服务，除非你愿意改（我也积极欢迎�?
...
�?Docker Hub �?GitHub Container Registry 拉取�?
...
Docker 部署包含以下特性：
- �?**自动数据库迁�?* - 容器启动时自动执�?
- �?**数据持久�?* - SQLite 数据库存储在主机�?`./zenblock-data/` 目录
- �?**健康检�?* - 容器�?0秒自动检查应用健康状�?
- �?**生产优化** - 使用 Next.js standalone 输出，镜像体积最�?
...
编辑 `messages/zh.json` �?`messages/en.json`�?
```json
{
  "ZenQuotes": [
    {
...
### Q: 数据会上传吗�?
**A:** 不会！所有数据存在本�?SQLite，只有你能看到自己有多菜�?

### Q: Docker 会保留我的数据吗�?
**A:** 会！数据库文件存储在主机�?`./zenblock-data/` 目录，容器重启或更新都不会丢失数据�?

### Q: 如何备份数据�?
**A:** 直接复制 `zenblock-data/` 目录。恢复时复制回去即可�?

### Q: 可以在服务器上运行吗�?
**A:** 当然可以！使�?Docker 部署效果最佳。记得开�?000端口并配置防火墙�?
...
**P.S.** 如果你看完这�?README 后依然能控制住自己不刷视频，恭喜你，你已经开始自律了！�?

**P.P.S.** 如果你用了这个工具还控制不住... 也许该考虑数字排毒旅行�?🏝�?
